...
Everything runs in your browser: all state (API keys, saved conversations, variables) lives in your browser's local storage, everything is done client-side, and third‑party libraries (KaTeX, Marked, Highlight.js) are pulled from public CDNs. But the list of all models of all providers is fetched from a json file hosted on my website (it grew too big for storing here to comfortable).
